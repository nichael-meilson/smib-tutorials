...
summary(regfit.fwd)
plot(summary(regfit.fwd)$bic,type="b")#we select 6 variables
...
par(mfrow=c(2,3))
plot(gam1,se=TRUE,col="purple")#Room.board mostly linear, PhD non-linear effect has v. high error, alumni linear, expend looks non-linear, grad.rate mostly linear
...
gam2 = gam(Outstate~Private +Room.Board +PhD+ perc.alumni+s(Expend,4)+Grad.Rate ,data=College,subset=train) 
...
lm1 = lm(Outstate~Private +Room.Board +PhD+ perc.alumni+Expend+Grad.Rate ,data=College,subset=train)
predlm1 = predict(lm1, newdata=College[test,]) 
mselm1 = mean((predlm1-College[test,"Outstate"])^2)#worse as expected

lm2 = lm(Outstate~Private +Room.Board +PhD+ perc.alumni+ns(Expend,4)+Grad.Rate ,data=College,subset=train)
