...
## Odds ratios
exp(donner.log$coefficients) # e^B0, e^B1, e^B2
exp(confint(donner.log))
exp(cbind(OR=donner.log$coefficients, confint(donner.log)))

# Odd ratio for survival after 10 years increase
...
c1=coef(donner.log)
plot(donner.na$Age,jitter(donner.na$Outcome, .2), pch=20,
     cex=1.2, xlab = "Age",ylab = "Status (jittered)")
curve(ilogit(c1[1]+c1[2]*x+c1[3]*0,0,1), add=T)
curve(ilogit(c1[1]+c1[2]*x+c1[3]*1,0,1), add=T, col="red")
...
donner.modnames<-c("Age","Sex","Age+Sex","Age*Sex")

# Akaike weights w/ AICcmodavg
